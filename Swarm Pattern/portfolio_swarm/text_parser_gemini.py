...
Uses latest google.genai (non-deprecated) with Gemini 2.5+
"""
import re
import json
import os
...
class DynamicKnowledgeBase:
    ...
    def __init__(self, cache_file: str = "portfolio_knowledge_cache.json"):
        ...
        if GEMINI_AVAILABLE and GeminiConfig.validate():
            # Determine which model to use (Gemini 2.5+ only)
            model_name = GeminiConfig.MODEL if hasattr(GeminiConfig, 'MODEL') else 'gemini-2.0-flash-exp'
            
            # Ensure we're using 2.5+ or 2.0+ models
            if 'gemini-2.5' not in model_name.lower() and 'gemini-2.0' not in model_name.lower():
                model_name = 'gemini-2.0-flash-exp'  # Default to latest stable
            
            try:
                # Try new google.genai API first (non-deprecated)
                if types is not None:
                    client = genai.Client(api_key=GeminiConfig.API_KEY)
                    self.gemini_model = client
                    self.model_name = model_name
                    self.use_new_api = True
                    logger.info(f"✅ Using latest google.genai API with {model_name}")
                else:
                    # Fallback to old API
                    genai.configure(api_key=GeminiConfig.API_KEY)
                    self.gemini_model = genai.GenerativeModel(model_name)
                    self.use_new_api = False
                    logger.warning("⚠️ Using deprecated google.generativeai - please upgrade to google.genai")
