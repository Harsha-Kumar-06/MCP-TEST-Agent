...
print("Testing all patterns on MSFT line:")
msft_line = "8000 MSFT shares at $410, paid $305"
for i, pattern in enumerate(position_patterns, 1):
    matches = list(re.finditer(pattern, msft_line, re.IGNORECASE | re.DOTALL))
    if matches:
        for m in matches:
            print(f"Pattern {i}: {m.groups()}")

print("\n\nTesting patterns on full text:")
for i, pattern in enumerate(position_patterns, 1):
    matches = list(re.finditer(pattern, text, re.IGNORECASE | re.DOTALL))
    msft_matches = [m for m in matches if 'msft' in m.group(0).lower()]
    if msft_matches:
        for m in msft_matches:
            print(f"Pattern {i} matched MSFT: {m.groups()}")
